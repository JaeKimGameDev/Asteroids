...
public class AllBallsGrow : MonoBehaviour
{
    // Ball Properties
    ...
    void FixedUpdate()
    {
        ...
        if (getData.timer <= 0)
        {
            ...
            SceneManager.LoadScene("LostLifeAllBallGrow");
            ...
            if (hit.collider != null)
            {
                ...
                if (hit.collider.tag == "CircleBall")
                {
                    ...
                    foreach (GameObject singleBall in ballList)
                    {
                        ...
                        temp = singleBall.transform.localScale;

                        temp.x += Time.deltaTime * 1.5f;
                        temp.y += Time.deltaTime * 1.5f;

                        singleBall.transform.localScale = temp;
                    }

                    localLevelScore += Time.deltaTime * 12;
                    scoreTop.text = ("" + localLevelScore.ToString("F0"));
                    scoreBottom.text = ("" + localLevelScore.ToString("F0"));
                }
                if (localLevelScore >= 250)
                {
                    SceneManager.LoadScene("IntermissionAllBallGrow");
                }
            }
        }
    }
