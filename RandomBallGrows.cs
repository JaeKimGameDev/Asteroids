...
public class RandomBallGrows : MonoBehaviour
{
    // Ball Properties
    ...
    void FixedUpdate()
    {
        ...
        if (getData.timer <= 0)
        {
            ...
            SceneManager.LoadScene("LostLifeRandomBall");
        }

        // using the game timer, randomly select ball to grow in array
        if ((getData.timer % 4) <= 1)
        {
            randomBall = ballList[Random.Range(0, ballList.Length)];
        }

        // Get input touches, supports multiple touches
        for (int i = 0; i < Input.touchCount; i++)
        {
            ...
            if (hit.collider != null)
            {
                ...
                if (hit.collider.tag == "CircleBall")
                {
                    ...
                    {
                        //otherBall = randomBall.GetComponent<Collider2D>();
                        //add "otherBall.IsTouching(singleBall.GetComponent<CircleCollider2D>()) ||"
                        //if you want to check for both "touched" and "random" ball for collision rule
                    ...
                    temp = randomBall.transform.localScale;

                    //Increase/increment x and y value, Time.deltaTime makes for a smoother transition for ball growth
                    temp.x += Time.deltaTime * 2.2f;
                    temp.y += Time.deltaTime * 2.2f;

                    randomBall.transform.localScale = temp;
                }
                localLevelScore += Time.deltaTime * 10;
                scoreTop.text = ("" + localLevelScore.ToString("F0"));
                scoreBottom.text = ("" + localLevelScore.ToString("F0"));
            }
            if (localLevelScore >= 100)
            {
                SceneManager.LoadScene("IntermissionRandomBall");
            }
        }
    }
}